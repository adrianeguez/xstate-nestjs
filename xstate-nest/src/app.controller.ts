...
export class AppController {
  ...
  ejemploMaquina(@Param('numero') numero: string): any {
    return this.appService.empezarMaquinaEjemplo(+numero);
  }

  @Get('ejemploMaquina/cargar/:estadoPrevio')
  empezarMaquinaEnAlgunPunto(@Param('estadoPrevio') estadoPrevio: string): any {
    return this.appService.empezarMaquinaEnAlgunPunto(estadoPrevio);
  }
  @Get('toggle/:event')
  toggleEvent(@Param('event') event: string): any {
    console.log(event);
    return this.appService.toggle(event);
  }

  @Get('toggle/:event/:id')
  toggleWithId(@Param('event') event: string, @Param('id') id: string): any {
    console.log(event);
  ...
  async microUno(): Promise<any> {
    const response = await firstValueFrom(
      this._clientProxyUMicroUno.send(MicrounoMessagesEnum.Transaccion, {
        id: 1,
      }),
    );
