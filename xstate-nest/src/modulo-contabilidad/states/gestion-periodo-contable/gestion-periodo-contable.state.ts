...
import {ResGpcA002, ResGpcA002Done} from "./respuestas/gpc-a-002.respuesta-done";
import {ResGpcA001, ResGpcA001Done} from "./respuestas/gpc-a-001.respuesta-done";
import {ResGpcA003, ResGpcA003Done} from "./respuestas/gpc-a-003.respuesta-done";
...
export class GestionPeriodoContableState {
    ...
    instanciaGestionPeriodoState = (contexto: GpcContexto) => createMachine<GpcContexto, GpcEventos, GpcTipoEstado>({
        ...
        states: {
                    ...
                    src: (context, event) => {
                        ...
                        return new Promise<void>(
                            (res, rej) => {
                                ...
                                console.log(`Validando anio repetido con ID periodo contable ${context.periodoContableId} y anio ${context.anio}`)
                                res();// OnDone
            ...
            CrearPeriodoContableConValoresSaldosCreditos: {
                invoke: {
                    ...
                    src: (context, event) => {
                        return new Promise<any>( // tipo periodo contable
                            (res, rej) => {
            ...
            CrearPeriodoContableYCuentasContableEnCero:{
                invoke: {
                    ...
                    src: (context, event) => {
                        return new Promise<ResGpcA001>( // tipo periodo contable
                            (res, rej) => {
                    ...
                    onDone: {
                        target: 'DeshabilitarPeriodoContableAnteriorYActivarNuevoPeriodoContable',
                        actions: assign<
                            GpcContexto,
                            ResGpcA001Done
                        >({
                            periodoContable: (context, event) => event.data.periodoContable,
                            cuentasContables: (context, event) => event.data.cuentasContables,
