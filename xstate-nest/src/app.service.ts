...
import {EjemploRetryPrnError, EjemploRetryPrnRespuestaError} from "./xstate/ejemplo-retry.prn.respuesta-error";
...
export class AppService {
    ...
    ejemploRetry = (parametroNumero: number) => createMachine<EjemploRetryContexto, EjemploRetryEventos, EjemploRetryTipoEstado>({
        ...
        states: {
            ...
            promesaRevisionNumero: {
                invoke: {
                    ...
                    src: (context, event) => {
                        console.log({message: `tiene un delay de: ${context.delay}`, context, event: event.type})
                        return new Promise<EjemploRetryPrnRespuesta>(
                            (res, rej) => {
                                setTimeout(
                                    () => {
                    ...
                    onDone: {
                        ...
                        actions: assign<
                            ...
                            EjemploRetryPrnRespuestaDone
                        >({
                            numeroEsMayorADiezYEsPar: (context, event) => event.data.numeroEsMayorADiezYEsPar,
                            revisionNumeroCompletado: (context, event) => event.data.revisionNumeroCompletado
                        })
                    },
                    onError: {
                        target: 'revisarRespuestaPromesaRevision',
                ...
                invoke: {
                    ...
                    src: (context, event) => {
                        return new Promise<void>(
                            (res, rej) => {
                                console.log(`Revisando si numero es par ${context.revisionNumeroCompletado}`);
                                if (context.revisionNumeroCompletado) {
                                    console.log(`Numero ES par ${context.revisionNumeroCompletado}`);
                                    res()
                                } else {
                                    console.log(`Numero NO ES par ${context.revisionNumeroCompletado}`);
                                    rej();
                                }
                            }
                        )
            ...
            retryPromesaRevisionNumero: {
                invoke: {
                    ...
                    src: (context, event) => {
                        return new Promise<void>(
                            (res, rej) => {
                                console.log(`Revisando maximo de retries. Actual: ${context.numeroRetries} Maximo: ${this.numeroMaximoRetries}`);
